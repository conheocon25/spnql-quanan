...
class Customer extends Mapper implements \MVC\Domain\CustomerFinder {
    ...
    function __construct() {
        ...
        $this->selectAllStmt = self::$PDO->prepare( 
                            "select * from muoiotxanh_customer");
        $this->selectStmt = self::$PDO->prepare( 
                            "select * from muoiotxanh_customer where id=?");
        $this->updateStmt = self::$PDO->prepare( 
                            "update muoiotxanh_customer set name=?, type=?, card=?, phone=?, address=?, note=?, discount=? where id=?");
        $this->insertStmt = self::$PDO->prepare( 
                            "insert into muoiotxanh_customer (name, type, card, phone, address, note, discount) 
							values( ?, ?, ?, ?, ?, ?, ?)");
		$this->deleteStmt = self::$PDO->prepare( 
                            "delete from muoiotxanh_customer where id=?");
		$this->findByPositionStmt = self::$PDO->prepare("
						SELECT id 
						FROM muoiotxanh_customer
						WHERE idlocation=?
						LIMIT ?,1
						ORDER By id asc
		");
		$this->findByCardStmt = self::$PDO->prepare("select * from muoiotxanh_customer where card=?");
		
		$tblCustomer = "muoiotxanh_customer";
	...
	function findByPostion($values) {		
        $str = "SELECT id FROM muoiotxanh_customer ORDER BY id LIMIT ". $values[0] .",1";		
